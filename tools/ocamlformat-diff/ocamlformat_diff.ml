...
let run_ocamlformat cmd ~ctx ~file_in =
  Bos.OS.File.tmp "%s" >>= fun path_out ->
  Bos.Cmd.(
...
let diff_in_ctx ~ctx ~old_file ~new_file ~diff =
  Bos.OS.Cmd.resolve (Bos.Cmd.v "ocamlformat") >>= fun fmt ->
  run_ocamlformat fmt ~ctx ~file_in:old_file >>= fun tmp_1 ->
  run_ocamlformat fmt ~ctx ~file_in:new_file >>= fun tmp_2 ->
  Bos.OS.Cmd.must_exist (Bos.Cmd.v diff) >>= fun diff ->
...
let diff_prelude path _ old_hex old_mode _ new_hex _ =
  let new_hex =
    match
      Bos.OS.Cmd.resolve (Bos.Cmd.v "git") >>= fun git ->
...
let run path old_file old_hex old_mode new_file new_hex new_mode diff =
  let diff =
    Bos.OS.File.must_exist (Fpath.v old_file) >>= fun old_file ->
    Bos.OS.File.must_exist (Fpath.v new_file) >>= fun new_file ->
