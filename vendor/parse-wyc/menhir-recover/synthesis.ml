...
module Synthesizer (G : GRAMMAR) (A : ATTRIBUTES with module G = G) :
  ...
  type variable = Head of lr1 * nonterminal | Tail of lr1 * production * int

  let variable_to_string = function
    | Head (st, n) ->
        Printf.sprintf "Head (#%d, %s)" (Lr1.to_int st) (Nonterminal.name n)
    | Tail (st, prod, pos) ->
        Printf.sprintf "Tail (#%d, p%d, %d)" (Lr1.to_int st)
  ...
  let cost_of = function
    ...
    | Tail (st, prod, pos) ->
        ...
        if Cost.is_infinite penalty then const bottom
          ...
          in
          const
            ( if can_reduce then (cost_of_prod prod, [ Reduce prod ])
            else (Cost.infinite, [ Abort ]) )
        else
          let head =
            let sym, _, _ = (Production.rhs prod).(pos) in
            let cost = cost_of_symbol sym in
            if Cost.is_infinite cost then
              match sym with T _ -> const bottom | N n -> var (Head (st, n))
