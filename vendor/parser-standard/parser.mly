...
let local_if_has_flags flags sloc x =
...
let_binding_body_no_punning:
  ...
  | optional_local let_ident type_constraint EQUAL seq_expr
      { let v = $2 in (* PR#7344 *)
        let t =
          ...
          | _ -> assert false
        in
        let pat = local_if Pattern $1 $loc($1) v in
  ...
  | optional_local let_ident COLON poly(core_type) EQUAL seq_expr
    {
      ...
      in
      let pat = local_if Pattern $1 $loc($1) $2 in
  ...
  | let_ident COLON TYPE newtypes DOT core_type EQUAL seq_expr
    { let exp, poly =
      ...
      let constraint' =
        Pvc_constraint { locally_abstract_univars=List.map fst $4; typ = poly}
      in
      ($1, exp, Some constraint') }
...
strict_function_or_labeled_tuple_type:
  | mktyp(
      ...
      codomain = strict_function_or_labeled_tuple_type
        { Ptyp_arrow(label,
                     local_if_has_flags unique_local $loc(unique_local) domain,
                     codomain) }
    )
    { $1 }
  | mktyp(
      ...
      %prec MINUSGREATER
        { Ptyp_arrow(label,
            local_if_has_flags arg_local $loc(arg_local) domain,
            local_if_has_flags ret_local $loc(ret_local)
      ...
      codomain = strict_function_or_labeled_tuple_type
         {
           ...
           Ptyp_arrow(label, local_if_has_flags local $loc(local) domain , codomain) }
    )
    { $1 }
  | mktyp(
      label = LIDENT COLON
