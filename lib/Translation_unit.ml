...
module Error = struct
  include Translation_unit_kernel.Error
  ...
  let print ?(debug = false) ?(quiet = false) fmt = function
    ...
    | Ocamlformat_bug {exn; input_name} -> (
        Format.fprintf fmt
          ...
          exe input_name ;
        match exn with
        | Translation_unit_kernel.Internal_error (m, l) ->
            let s =
              match m with
...
let parse_and_format = Translation_unit_kernel.parse_and_format
