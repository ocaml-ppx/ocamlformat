...
module Normalize = struct
        ...
        let txt =
          try
            let ({ast; comments; _} : _ Parse_with_comments.with_comments) =
              Parse_with_comments.parse Parse.ast Structure c.conf
                ~source:txt
            in
  ...
  let moved_docstrings fragment c s1 s2 =
    let c = {conf= c; normalize_code= ast Structure c} in
  ...
  let docstring conf =
    let normalize_code = ast Structure conf in
  ...
  let diff_cmts (conf : Conf.t) x y =
    let norm z =
      let norm_non_code {Cmt.txt; _} = comment txt in
      let f z =
