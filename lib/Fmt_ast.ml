  ...
  | Ptyp_object (fields, closedness) ->
      let fmt_field {pof_desc; pof_attributes= atrs; pof_loc} =
        ...
        @@ hvbox 4
             ...
             $ fmt_attributes c ~pre:(pre_attributes c) ~key:"@" atrs )
      in
      hvbox 0
        (wrap "< " " >"
...
and fmt_expression c ?(box = true) ?pro ?epi ?eol ?parens ?(indent_wrap = 0)
  ...
  let fmt_atrs =
    fmt_attributes c ~pre:(pre_attributes c) ~key:"@" pexp_attributes
  in
...
and fmt_class_expr c ?eol ?(box = true) ({ast= exp; _} as xexp) =
  ...
  let fmt_atrs =
    fmt_attributes c ~pre:(pre_attributes c) ~key:"@" pcl_attributes
  in
...
and fmt_class_field c ctx (cf : class_field) =
  ...
  in
  let fmt_atrs = fmt_attributes c ~pre:(pre_attributes c) ~key:"@@" atrs in
