...
module Indexing_op = struct
    ...
    | ident, args -> (
        ...
        | ident :: path_rev -> (
            ...
            | Some { opchars; brackets; extended; has_rhs } -> (
                ...
                | true, [ arg1; rhs ] ->
                    custom ~extended ~rhs:(Some rhs) op arg1
                | false, [ arg1 ] ->
                    custom ~extended ~rhs:None op arg1
                | _, _ ->
                    None ) ) )
  ...
  let rec all_args_unlabeled acc = function
    | [] ->
        Some (List.rev acc)
    | (Asttypes.Nolabel, e) :: tl ->
        all_args_unlabeled (e :: acc) tl
    | _ :: _ ->
        None
...
module String_id = struct
  ...
  let is_hash_getter i =
    ...
    | '#', ('#' | '.') when String.for_all i ~f:is_infix_char ->
        true
    | _ ->
        false
...
module Attr = struct
  let is_doc = function
    | { attr_name= { Location.txt= "ocaml.doc" | "ocaml.text"; _ }; _ } ->
        true
    | _ ->
        false
end
...
module Exp = struct
  let test_id ~f = function
    | { pexp_desc= Pexp_ident { txt= i; _ }; _ } ->
        f i
    | _ ->
        false
  ...
  let rec is_trivial c exp =
    match exp.pexp_desc with
    | Pexp_constant { pconst_desc= Pconst_string (_, _, None); _ } ->
        true
    | Pexp_constant _ | Pexp_field _ | Pexp_ident _ | Pexp_send _ ->
        true
    | Pexp_construct (_, exp) ->
        Option.for_all exp ~f:(is_trivial c)
    | Pexp_apply (e0, [ (_, e1) ]) when is_prefix e0 ->
        is_trivial c e1
    | Pexp_apply
...
let rec mty_is_simple x =
  ...
  | Pmty_ident _ | Pmty_alias _ | Pmty_signature [] ->
      true
  | Pmty_signature (_ :: _)
  | Pmty_with (_, _ :: _ :: _)
  | Pmty_extension _
...
and mod_is_simple x =
  ...
  | Pmod_ident _ | Pmod_unpack _ | Pmod_structure [] | Pmod_hole ->
      true
  | Pmod_structure (_ :: _) | Pmod_extension _ | Pmod_functor (_, _) ->
      false
  | Pmod_constraint (e, t) ->
      mod_is_simple e && mty_is_simple t
  | Pmod_apply (a, b) ->
      mod_is_simple a && mod_is_simple b
...
module Structure_item = struct
  let has_doc itm =
    ...
    | Pstr_attribute atr ->
        Option.is_some (fst (doc_atrs [ atr ]))
    | Pstr_eval (_, atrs)
...
module Signature_item = struct
  let has_doc itm =
    ...
    | Psig_recmodule
        ...
        :: _ )
    | Psig_include
        { pincl_mod= { pmty_attributes= atrs1; _ }
        ; pincl_attributes= atrs2
        ; _
        }
    | Psig_exception
...
let break_between cc (i1, c1) (i2, c2) =
  ...
  | Sig i1, Sig i2 ->
      Signature_item.break_between cc (i1, c1) (i2, c2)
  | Vb i1, Vb i2 ->
      Vb.break_between cc (i1, c1) (i2, c2)
  | Mty _, Mty _ ->
      break_between_modules cc (i1, c1) (i2, c2)
  | Mod _, Mod _ ->
      break_between_modules cc (i1, c1) (i2, c2)
  ...
  | Clf i1, Clf i2 ->
      Class_field.break_between cc (i1, c1) (i2, c2)
  | Ctf i1, Ctf i2 ->
      Class_type_field.break_between cc (i1, c1) (i2, c2)
  | Td i1, Td i2 ->
      Td.break_between cc (i1, c1) (i2, c2)
  | _ ->
      assert false
...
and Requires_sub_terms : sig
  ...
  let check_typ { ctx; ast= typ } =
    ...
    let check_ext { pext_kind; _ } =
      ...
      | Pext_decl (_, cstr, t0) ->
          check_cstr cstr || Option.exists t0 ~f
      | _ ->
          false
    in
    ...
    in
    let check_type
        { ptype_params; ptype_cstrs; ptype_kind; ptype_manifest; _ } =
    ...
    let check_pcstr_fields pcstr_fields =
      List.exists pcstr_fields ~f:(fun f ->
          ...
          | Pcf_val
              ( _
              , _
              , Cfk_concrete (_, { pexp_desc= Pexp_constraint (_, t); _ }) )
            ->
              typ == t
          | Pcf_val (_, _, Cfk_concrete _) ->
              false
          | Pcf_method (_, _, Cfk_virtual t) ->
              typ == t
          | Pcf_method
              ( _
              , _
          ...
          | Pcf_method
            ...
            ->
              let rec loop = function
                | { pexp_desc= Pexp_newtype (_, e); _ } ->
                    loop e
                | { pexp_desc= Pexp_constraint (_, t); _ } ->
                    t == typ
                | { pexp_desc= Pexp_fun (_, _, _, e); _ } ->
                    loop e
                | _ ->
                    false
              in
          ...
          | Pcf_constraint (t1, t2) ->
              t1 == typ || t2 == typ
          | Pcf_initializer _ | Pcf_attribute _ | Pcf_extension _ ->
              false )
    in
    ...
    | Typ ctx -> (
        ...
        | Ptyp_any | Ptyp_var _ ->
            assert false
        | Ptyp_alias (t1, _) | Ptyp_poly (_, t1) ->
            assert (typ == t1)
        | Ptyp_arrow (_, t1, t2) ->
            assert (typ == t1 || typ == t2)
        | Ptyp_tuple t1N | Ptyp_constr (_, t1N) ->
            assert (List.exists t1N ~f)
        | Ptyp_variant (r1N, _, _) ->
            assert (
              List.exists r1N ~f:(function
                | { prf_desc= Rtag (_, _, t1N); _ } ->
                    List.exists t1N ~f
                | { prf_desc= Rinherit t1; _ } ->
                    typ == t1 ) )
        | Ptyp_package (_, it1N) ->
        ...
        | Ptyp_object (fields, _) ->
            assert (
              List.exists fields ~f:(function
                | { pof_desc= Otag (_, t1); _ } ->
                    typ == t1
                | { pof_desc= Oinherit t1; _ } ->
                    typ == t1 ) )
        | Ptyp_class (_, l) ->
            assert (List.exists l ~f) )
    | Td { ptype_manifest; _ } -> (
    ...
    | Pat ctx -> (
        ...
        | Ppat_constraint (_, t1) ->
            assert (typ == t1)
        | Ppat_extension (_, PTyp t) ->
            assert (typ == t)
        | _ ->
            assert false )
    | Exp ctx -> (
        ...
        | Pexp_constraint
            (_, ({ ptyp_desc= Ptyp_package (_, it1N); _ } as ty)) ->
        ...
        | Pexp_extension (_, PTyp t1) ->
            assert (typ == t1)
        | Pexp_coerce (_, Some t1, t2) ->
            assert (typ == t1 || typ == t2)
        | Pexp_letexception (ext, _) ->
            assert (check_ext ext)
    ...
    | Sig ctx -> (
        ...
        | Psig_value { pval_type= t1; _ } ->
            assert (typ == t1)
        | Psig_type (_, d1N) ->
            assert (List.exists d1N ~f:check_type)
        | Psig_typesubst d1N ->
            assert (List.exists d1N ~f:check_type)
        | Psig_typext typext ->
            assert (check_typext typext)
        | Psig_exception ext ->
            assert (check_typexn ext)
        | Psig_class_type l ->
            assert (check_class_type l)
        | Psig_class l ->
            assert (check_class_type l)
        | _ ->
            assert false )
    | Str ctx -> (
        ...
        | Pstr_class l ->
            assert (
              List.exists l
                ~f:(fun { pci_expr= { pcl_desc; _ }; pci_params; _ } ->
                  List.exists pci_params ~f:(fun (t, _) -> t == typ)
                  ||
                  match pcl_desc with
                  | Pcl_constr (_, l) ->
  ...
  let check_pat { ctx; ast= pat } =
    ...
    | Pat ctx -> (
        ...
        | Ppat_record (p1N, _) ->
            assert (List.exists p1N ~f:snd_f)
        | Ppat_construct
            ( { txt= Lident "::"; _ }
  ...
  let check_exp { ctx; ast= exp } =
    ...
    | Exp ctx -> (
        ...
        | Pexp_let (_, bindings, e) ->
            assert (
              List.exists bindings ~f:(fun { pvb_expr; _ } ->
                  pvb_expr == exp )
              || e == exp )
        ...
        | Pexp_function cases | Pexp_match (_, cases) | Pexp_try (_, cases)
          ->
            assert (
              List.exists cases ~f:(function
                | { pc_guard= Some g; _ } when g == exp ->
                    true
                | { pc_rhs; _ } when pc_rhs == exp ->
                    true
                | _ ->
                    false ) )
        ...
        | Pexp_apply (e0, args)
          when Option.is_some (Indexing_op.get_sugar e0 args) ->
            ...
            let in_args =
              ...
              | Defined (e2, _) ->
                  e2 == exp
              | Extended (args, _) | Special (args, _) ->
                  List.exists args ~f
            in
        ...
        | Pexp_override e1N ->
            assert (List.exists e1N ~f:snd_f) )
    | Vb vb ->
        assert (vb.pvb_expr == exp)
    | Str str -> (
        match str.pstr_desc with
        | Pstr_eval (e0, _) ->
            assert (e0 == exp)
  ...
  let prec_ctx ctx =
    ...
    | { ctx=
          ( Str
              { pstr_desc= Pstr_exception { ptyexn_constructor= constr; _ }
              ; _
              }
          | Sig
              { psig_desc= Psig_exception { ptyexn_constructor= constr; _ }
              ; _
              }
    ...
    | { ctx= Exp { pexp_desc; _ }; ast= Exp exp } -> (
        ...
        | Pexp_construct
            ( { txt= Lident "::"; _ }
            , Some { pexp_desc= Pexp_tuple [ _; e2 ]; _ } ) ->
        ...
        | Pexp_variant (_, Some _) ->
            Some (Apply, Non)
        | Pexp_apply
            ({ pexp_desc= Pexp_ident { txt= Lident i; loc }; _ }, [ _ ]) -> (
            match i with
            | "~-" | "~-." | "~+" | "~+." ->
                if
        ...
        | Pexp_apply
            ...
            match (i.[0], i) with
            | _, ":=" ->
                Some (ColonEqual, child)
            | _, ("or" | "||") ->
                Some (BarBar, child)
            | _, ("&" | "&&") ->
                Some (AmperAmper, child)
            | ('=' | '<' | '>' | '|' | '&' | '$'), _ | _, "!=" ->
                Some (InfixOp0, child)
            | ('@' | '^'), _ ->
                Some (InfixOp1, child)
            ...
            | '#', _ ->
                Some (HashOp, child)
            | _ ->
                Some (Apply, if String_id.is_infix i then child else Non) )
        | Pexp_apply _ ->
            Some (Apply, Non)
        | Pexp_setfield (e0, _, _) when e0 == exp ->
            Some (Dot, Left)
        | Pexp_setfield (_, _, e0) when e0 == exp ->
            Some (LessMinus, Non)
        | Pexp_setinstvar _ ->
            Some (LessMinus, Non)
        | Pexp_field _ ->
  ...
  let rec prec_ast =
    ...
    | Exp { pexp_desc; _ } -> (
        ...
        | Pexp_apply
            ({ pexp_desc= Pexp_ident { txt= Lident i; loc; _ }; _ }, [ _ ])
          -> (
            match i with
            | "~-" | "~-." | "~+." | "~+" ->
                if
        ...
        | Pexp_apply
            ({ pexp_desc= Pexp_ident { txt= Lident i; _ }; _ }, [ _; _ ])
          -> (
            match (i.[0], i) with
            | _, ":=" ->
                Some ColonEqual
    ...
    | Some (prec_ctx, which_child) -> (
        ...
        | Some prec_ast ->
            let ambiguous =
              ...
              (* add parens only when the context has a higher prec than
                 ast *)
              | cmp ->
                  cmp >= 0
            in
  ...
  let parenze_typ ({ ctx; ast= typ } as xtyp) =
    ...
    match xtyp with
    | { ast= { ptyp_desc= Ptyp_package _; _ }; _ } ->
        true
    | { ast= { ptyp_desc= Ptyp_alias _; _ }; ctx= Typ _ } ->
        true
      ...
      when List.exists t ~f:(fun t ->
               ...
               | Ptype_variant l ->
                   List.exists l ~f:(fun c ->
                       ...
                       | Pcstr_tuple l ->
                           List.exists l ~f:(phys_equal typ)
                       | _ ->
                           false )
               | _ ->
                   false ) ->
  ...
  let parenze_mty { ctx; ast= mty } =
    ...
    | Str { pstr_desc= Pstr_recmodule _; _ }, Pmty_with _ ->
        true
    | Sig { psig_desc= Psig_recmodule _; _ }, Pmty_with _ ->
        true
    | _ ->
        false
  ...
  let parenze_mod { ctx; ast= m } =
    ...
    | Mod { pmod_desc= Pmod_apply (_, x); _ }, Pmod_functor _ when m == x ->
        false
    | Mod { pmod_desc= Pmod_apply _; _ }, Pmod_functor _ ->
        true
    | _ ->
        false
  ...
  let parenze_pat ({ ctx; ast= pat } as xpat) =
    ...
    | ( Pat
      ...
      , inner ) -> (
        ...
        | Ppat_construct ({ txt= Lident "::"; _ }, _) ->
            true
        | Ppat_construct _ | Ppat_record _ | Ppat_variant _ ->
            false
        | _ ->
            true )
  ...
  let is_displaced_prefix_op { ctx; ast= exp } =
    ...
    | ( Exp { pexp_desc= Pexp_apply (e0, [ (Nolabel, _) ]); _ }
      ...
      when e0 == exp && Longident.is_prefix i ->
        false
    | _, Pexp_ident { txt= i; _ } when Longident.is_prefix i ->
        true
    | _ ->
        false
  ...
  let rec exposed_right_exp =
    ...
    fun cls exp ->
      let exposed_ () =
        ...
        | Pexp_construct
            ( { txt= Lident "::"; _ }
            , Some { pexp_desc= Pexp_tuple [ _; e ]; _ } )
  ...
  and parenze_exp ({ ctx; ast= exp } as xexp) =
    ...
    let rec ifthenelse pexp_desc =
      ...
      | Pexp_let _ | Pexp_match _ | Pexp_try _ ->
          true
      | _ ->
          false
    in
    let exp_in_sequence lhs rhs exp =
      ...
      | (Pexp_match _ | Pexp_try _), _ :: _ when lhs == exp ->
          true
      | _, _ :: _ ->
          false
      | ( Pexp_extension
      ...
      | _ when rhs == exp ->
          false
      | _ ->
          failwith "exp must be lhs or rhs from the parent expression"
    in
    ...
    | Exp { pexp_desc= Pexp_object _; _ }, _ ->
        false
    ...
    | Exp { pexp_desc= Pexp_extension _; _ }, { pexp_desc= Pexp_tuple _; _ }
      ->
        false
    | Pld _, { pexp_desc= Pexp_tuple _; _ } ->
        false
    | Cl { pcl_desc= Pcl_apply _; _ }, _ ->
        parenze ()
    ...
    | Exp { pexp_desc= Pexp_ifthenelse (_, _, Some e); _ }, { pexp_desc; _ }
      when !parens_ite && e == exp && ifthenelse pexp_desc ->
        true
    | ( Exp
          { pexp_desc= Pexp_apply (op, (Nolabel, _) :: (Nolabel, e1) :: _)
          ; _
          }
      , { pexp_desc=
            Pexp_apply
              ({ pexp_desc= Pexp_ident { txt= Lident "not"; _ }; _ }, _)
        ; _
        } )
    ...
    | Exp { pexp_desc; _ }, _ -> (
        ...
        | Pexp_ifthenelse (cnd, _, _) when cnd == exp ->
            false
        ...
        | Pexp_record
            (_, Some ({ pexp_desc= Pexp_apply (ident, [ _ ]); _ } as e0))
        ...
        | _ -> (
            ...
            | Pexp_list _ ->
                false
            | _ ->
                Exp.has_trailing_attributes exp || parenze () ) )
    | _, { pexp_desc= Pexp_list _; _ } ->
        false
    | _, exp when Exp.has_trailing_attributes exp ->
        true
    | _ ->
        false
