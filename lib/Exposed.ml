...
module Right = struct
  ...
  let label_declaration = function
    | { pld_attributes= _ :: _; _ } ->
        false
    | { pld_type; _ } ->
        core_type pld_type

  let row_field = function
    | { prf_attributes= _ :: _; _ } ->
        false
  ...
  let signature_item { psig_desc; psig_loc= _ } =
    ...
    | Psig_value vd ->
        value_description vd
    | Psig_type (_recflag, typedecls) ->
        list ~elt:type_declaration typedecls
    | Psig_typesubst typedecls ->
        list ~elt:type_declaration typedecls
    | Psig_typext te ->
        type_extension te
    | Psig_exception te ->
  ...
  let payload = function
    | PStr items ->
        list ~elt:structure_item items
    | PSig items ->
        list ~elt:signature_item items
    | PTyp t ->
        core_type t
    | PPat _ ->
        false
