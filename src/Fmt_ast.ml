...
let fmt_longident_loc c ?(pre = ("" : _ format))
  ...
  Cmts.fmt c loc @@ (fmt pre $ fmt_longident txt)

let fmt_str_loc c ?(pre = ("" : _ format)) ({txt; loc} : string loc) =
  Cmts.fmt c loc @@ (fmt pre $ str txt)
            ...
            ( box_t
                ( hvbox_if (Option.is_some pro_t) 0
                    ...
                    $ hvbox 4
                        ...
                        $ list_pn arg_blks
                            (fun ?prev:_ (name, arg_mtyp) ?next ->
                              let maybe_box k =
                                match arg_mtyp with
                                | Some {pro= None} -> hvbox 2 k
                                | _ -> k
                              in
                              fmt "@ "
                              $ maybe_box
                                  (wrap "(" ")"
                                     ( fmt_str_loc c name
                                     $ opt arg_mtyp
                                         (fun {pro; psp; bdy; cls; esp; epi}
                                         ->
                                           fmt " : " $ Option.call ~f:pro
                                           $ fmt_if_k (Option.is_some pro)
                                               close_box
                                           $ psp $ bdy
                                           $ fmt_if_k (Option.is_some pro)
                                               cls
...
let fmt_signature s cmts c itms =
  ...
  Ast.init c.conf ;
  match itms with
  | [] -> Cmts.fmt_after c Location.none
