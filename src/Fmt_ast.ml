...
let fmt_longident_loc c ?(pre = fmt "") {txt; loc} =
  Cmts.fmt c loc @@ (pre $ fmt_longident txt)

let fmt_str_loc c ?(pre = fmt "") {txt; loc} =
...
let fmt_let c ctx ~ext ~rec_flag ~bindings ~body ~parens ~attributes
  ...
  let fmt_binding ~first ~last binding =
    ...
    let in_ indent = fmt_if_k last (break 1 (-indent) $ fmt "in") in
...
and fmt_core_type c ?(box = true) ?(in_type_declaration = false) ?pro
  ...
  | Ptyp_poly (a1N, t) ->
      hovbox_if box 0
        ( list a1N "@ " (fun ty -> fmt_str_loc c ~pre:(fmt "'") ty)
        $ fmt ".@ "
  ...
  | Ptyp_variant (rfs, flag, lbls) ->
      let row_fields rfs =
        match rfs with
        | [] -> Cmts.fmt_within c ~pro:(fmt "") ptyp_loc
        | _ ->
            list rfs
  ...
  | Ptyp_class (lid, []) -> fmt_longident_loc c ~pre:(fmt "#") lid
  | Ptyp_class (lid, [t1]) ->
        ...
        Cmts.fmt c lid.loc
          (hvbox 0
             (wrap_k (char opn) (char cls)
                (Cmts.fmt_within c ~pro:(fmt " ") ~epi:(fmt " ") ppat_loc)))
      ...
      hvbox 2
        (wrap_if parens "(" ")"
           ...
           $ ( match ctx0 with
             ...
             | _ -> fmt " :@ " )
           $ fmt_core_type c (sub_typ ~ctx typ) ))
  | Ppat_type lid -> fmt_longident_loc c ~pre:(fmt "#") lid
  | Ppat_lazy pat ->
      cbox 2
        (wrap_if parens "(" ")"
...
and fmt_fun_args c ?(pro = fmt "") args =
  let fmt_fun_arg (a : Sugar.arg_kind) =
    ...
    | Newtypes names ->
        cbox 0
          (wrap "(" ")" (fmt "type " $ list names "@ " (fmt_str_loc c)))
  in
  fmt_if_k
    ...
    | Lident (("()" | "[]") as txt) ->
        ...
        @@ hvbox 0
             (wrap_if
                ...
                "(" ")"
                ( wrap_k (char opn) (char cls)
                    (Cmts.fmt_within c ~pro:(fmt " ") ~epi:(fmt " ")
                       pexp_loc)
                $ fmt_atrs ))
    | Lident "::" ->
        wrap_if parens "(" ")"
  ...
  | Pexp_sequence _ ->
      ...
      let fmt_grp grp =
        list grp
          ...
          | `Separator when c.conf.break_sequences -> " ;@;<1000 0>"
          | `Separator -> " ;@ "
          | `Terminator when c.conf.break_sequences -> ";@;<1000 0>"
          | `Terminator -> ";@ " )
          (fmt_expression c)
      in
      hvbox 0
...
and fmt_type_declaration c ?(pre = "") ?(brk = fmt "") ctx ?fmt_name
  ...
  let fmt_manifest_kind mfst priv kind =
    ...
    | Ptype_record lbl_decls -> (
        let fmt_decl_break_after ~first:_ ~last x =
        ...
        | `Before ->
            ...
            $ fmt "@ "
            $ hvbox 0 (wrap_record c (list_fl lbl_decls fmt_decl))
        | `After ->
            box_manifest
              ( fmt_manifest ~priv:Public mfst
              $ fmt " =" $ fmt_private_flag priv )
            $ fmt "@ "
            $ hvbox 2
                (wrap_record c (list_fl lbl_decls fmt_decl_break_after))
        | `After_and_docked ->
            let space =
              if c.conf.space_around_collection_expressions then 1 else 0
            in
            box_manifest
              ( fmt_manifest ~priv:Public mfst
              $ fmt " =" $ fmt_private_flag priv $ fmt " {" )
            $ break space 0
            $ list_fl lbl_decls fmt_decl_break_after
            $ break space (-2) $ fmt "}" )
    | Ptype_open ->
        box_manifest
          ( fmt_manifest ~priv:Public mfst
  ...
  | Pcstr_record lds ->
      let fmt_ld ~first ~last x =
        ...
        | `After | `After_and_docked ->
            ...
            $ fmt_if (not last) "@;<1 2>"
      in
      pre $ wrap_record c (list_fl lds fmt_ld)
...
and fmt_extension_constructor c sep ctx ec =
  ...
  let suf =
    ...
    | Pext_decl ((Pcstr_tuple [] | Pcstr_record [] | _), None)
     |Pext_rebind _ ->
        fmt ""
    | Pext_decl ((Pcstr_tuple [] | Pcstr_record [] | _), Some _) -> fmt " "
  in
  Cmts.fmt c pext_loc
  @@ hvbox 4
       ( hvbox 2
