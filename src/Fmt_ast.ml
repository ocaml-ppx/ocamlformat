...
let sugar_fun c ?(will_keep_first_ast_node = true) xexp =
  let rec sugar_fun_ ?(will_keep_first_ast_node = false)
      ({ ast= exp } as xexp) =
    let ctx = Exp exp in
...
let rec sugar_list_pat c pat =
  ...
  | Ppat_construct
      ( { txt= Lident "::"; loc }
      , Some
          { ppat_desc= Ppat_tuple [ hd; tl ]; ppat_loc; ppat_attributes= [] }
      ) -> (
...
and fmt_pattern c ?pro ?parens ({ ctx= ctx0; ast= pat } as xpat) =
  ...
  | Ppat_construct ({ txt; loc }, _)
    when Poly.(txt <> Longident.Lident "::") ->
      fun k ->
...
and fmt_fun_args c ?(pro = fmt "") args =
  let fmt_fun_arg = function
    ...
    | Val
        ...
        , ({ ast=
               { ppat_desc=
                   ( Ppat_var { txt }
                   | Ppat_constraint
                       ( { ppat_desc= Ppat_var { txt }; ppat_attributes= [] }
                       , _ ) )
               ; ppat_attributes= []
               }
           } as xpat)
    ...
    | Val
        ( Optional l
        , ({ ast= { ppat_desc= Ppat_var { txt }; ppat_attributes= [] } } as
          xpat)
        , Some xexp )
      when String.equal l txt ->
        cbox 0
...
and fmt_expression c ?(box = true) ?epi ?eol ?parens ?ext
  ...
  let fmt_op_args op_args =
    ...
    let is_nested_diff_prec_infix_ops =
      let infix_prec ast =
        match ast with
        | Exp { pexp_desc= Pexp_apply (e, _) } when is_infix e ->
            prec_ast ast
        | Exp
            ({ pexp_desc=
                 Pexp_construct
  ...
  | Pexp_function cs ->
      wrap_if parens "(" ")"
        (hvbox 2 (fmt "function" $ fmt_attributes c ~key:"@" pexp_attributes)
        $ fmt "@ "
  ...
  | Pexp_letexception (ext_cstr, exp) ->
      hvbox 0
        (wrap_if
           ...
           "(" ")"
           (hvbox 0
              (fmt_exception ~pre:(fmt "let exception@ ") c (fmt ": ") ctx
                 ext_cstr
              $ fmt "@ in")
           $ fmt "@;<1000 0>"
           $ fmt_expression c (sub_exp ~ctx exp))
          ...
          [ ({ pstr_desc=
                 Pstr_eval
                   ( ({ pexp_desc=
                          ...
                          | Pexp_sequence _ | Pexp_new _ | Pexp_letmodule _
                          | Pexp_object _ )
                      ; pexp_attributes= []
                      } as e1)
                   , _ )
