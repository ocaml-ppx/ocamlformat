...
and fmt_pattern c ?pro ?parens ({ctx= ctx0; ast= pat} as xpat) =
  ...
  | Ppat_constraint
      ...
      , ( { ptyp_desc= Ptyp_package (id, cnstrs)
          ; ptyp_attributes= []
          ; ptyp_loc= (* TODO: use ptyp_loc *) _ } as typ ) ) ->
      let ctx = Typ typ in
      hovbox 0
        (wrap_if parens "(" ")"
           (hvbox 1
...
and fmt_expression c ?(box = true) ?pro ?epi ?eol ?parens ?(indent_wrap = 0)
  ...
  | Pexp_apply
      ...
      , [ (Nolabel, l)
        ; ( Nolabel
          , ({pexp_desc= Pexp_ident _; pexp_loc= _; pexp_attributes= _} as r)
          ) ] )
  ...
  | Pexp_apply (e0, e1N1) -> (
      ...
      | (lbl, ({pexp_desc= Pexp_fun _; pexp_loc; pexp_attributes= _} as eN1))
        :: rev_e1N
        ...
        when List.for_all rev_e1N ~f:(fun (_, eI) ->
          ...
          hvbox 2
            (wrap_if parens "(" ")"
               ...
               $ fmt_atrs ))
      | ( lbl
        , ({pexp_desc= Pexp_function cs; pexp_loc; pexp_attributes= _} as eN)
        )
        :: rev_e1N
        when List.for_all rev_e1N ~f:(fun (_, eI) ->
                 is_simple c.conf (fun _ -> 0) (sub_exp ~ctx eI)) ->
  ...
  | Pexp_construct
      ( {txt= Lident "::"; loc= _}
      , Some {pexp_desc= Pexp_tuple [_; _]; pexp_attributes= []; pexp_loc= _}
      ) -> (
...
and fmt_class_expr c ?eol ?(box = true) ({ast= exp; _} as xexp) =
  ...
  | Pcl_fun _ ->
      ...
      let args_box =
        if Poly.(c.conf.break_fun_decl = `Fit_or_vertical) then hvbox
        else hovbox
      in
      hvbox_if box
  ...
  let fmt_kind = function
    ...
    | Cfk_concrete
        ...
        | None ->
            ...
            , fmt "@ " $ fmt_expression c (sub_exp ~ctx e) ) )
    | Cfk_concrete
        (_, {pexp_desc= Pexp_poly (e, poly); pexp_loc; pexp_attributes= _})
      ->
        let xargs, xbody =
          match poly with
          | None ->
