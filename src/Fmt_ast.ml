...
and fmt_core_type c ?(box = true) ?(in_type_declaration = false) ?pro
  ...
  | Ptyp_poly (a1N, t) ->
      hovbox_if box 0
        ( list a1N "@ " (fmt_str_loc c ~pre:(fmt "'"))
        $ fmt ".@ "
...
and fmt_body c ?ext ({ast= body} as xbody) =
  ...
  | _ -> (fmt "", fmt "@ " $ fmt_expression c ~eol:(fmt "@;<1000 0>") xbody)
           ...
           ( hvbox 2
               (wrap "[" "]"
                  ...
                  $ hovbox 2
                      ...
                      $ fmt_attributes c ~suf:(fmt " ") call.pexp_attributes
                          ~key:"@"
        ...
        (wrap_fits_breaks_if ~space:false c.conf parens "(" ")"
           ...
           $ hvbox 2
               (wrap "[" "]"
                  ...
                  $ hovbox 2
                      ( fmt_str_loc c name $ fmt " fun "
                      $ fmt_attributes c ~suf:(fmt " ") retn.pexp_attributes
                          ~key:"@"
                      $ fmt_fun_args c xargs $ fmt "@ ->" )
                  $ fmt "@ " $ fmt_expression c xbody )) ))
  | Pexp_apply
  ...
  | Pexp_letexception (ext_cstr, exp) ->
      ...
      hvbox 0
        ( wrap_if
            ...
            "(" ")"
            ( hvbox 0
                (fmt_exception ~pre c (fmt ": ") ctx ext_cstr $ fmt "@ in")
            $ fmt "@;<1000 0>"
            $ fmt_expression c (sub_exp ~ctx exp) )
        $ fmt_atrs )
  ...
  let fmt_manifest_kind mfst priv kind =
    ...
    | Ptype_record lbl_decls ->
        ...
        box_manifest
          ...
          $ fmt " =" $ fmt_private_flag priv $ p.docked_before )
        $ p.break_before
