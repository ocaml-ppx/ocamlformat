...
let (_profile : Conf.t option C.t) =
  let doc =
...
let () = Conf.action := fun () -> action

let () = Conf.debug := debug

let parse_line_in_attribute = parse_line ~from:`Attribute

let () = Conf.parse_line_in_attribute := parse_line_in_attribute
