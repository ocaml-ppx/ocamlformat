...
let format ?output_file ~kind ~input_name ~source (conf : Conf.t) =
  if conf.fmt_opts.disable then Ok source
  else
...
let print_error (conf : Conf.t) e =
  Translation_unit.Error.print Format.err_formatter
    ~debug:conf.opr_opts.debug ~quiet:conf.fmt_opts.quiet e
...
let run_action action =
  ...
  | Numeric ({kind; file; name= input_name; conf}, range) ->
      let conf = {conf with fmt_opts= {conf.fmt_opts with quiet= true}} in
      let source = source_from_file file in
