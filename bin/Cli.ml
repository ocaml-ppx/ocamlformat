...
module C : sig
  type config = Ocamlformat_api.Conf.t
...
let (_profile : Ocamlformat_api.Conf.t option C.t) =
  let doc =
  ...
  | [s] -> (
    ...
    | [name; value], `File (filename, lnum) ->
        (* tolerate space separated [var value] to compatibility with older
           config file format *)
        if not config.Ocamlformat_api.Conf.quiet then
          Format.eprintf
            "File %a, line %d:\n\
