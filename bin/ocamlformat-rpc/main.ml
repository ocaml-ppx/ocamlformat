              ...
              List.fold_until ~init:()
                ...
                ~f:(fun () try_formatting ->
                  ...
                  | Error e ->
                      ...
                      Continue () )
                (* The formatting functions are ordered in such a way that
                   the ones expecting a keyword first (like signatures) are
                   placed before the more general ones (like toplevel
                   phrases). Parsing a file as `--impl` with `ocamlformat`
                   processes it as a use file (toplevel phrases) anyway.
          ...
          | `Config c -> (
              ...
              | Error e ->
                  let msg =
                    ...
                    | `Misplaced (x, y) ->
                        Format.sprintf
                          "Misplaced configuration value (%s, %s)" x y
                    | `Unknown (x, _) ->
